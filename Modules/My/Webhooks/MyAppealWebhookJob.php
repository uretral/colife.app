...
class MyAppealWebhookJob extends ProcessWebhookJob
{
    ...
    public function handle()
    {
        try {
            ...
            $supportId = Helper::addMySupportToAppeal($appeal);
            Helper::storeMyAppealMessage($message, $appeal, $supportId);

            event(new AppealUpdatedEvent($appeal->id));
            $appeal->users()->each( fn(User $user) => event(new MyNotificationEvent($user->id)) );
