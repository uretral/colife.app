...
use App\Services\Bitrix\Enum\ApartmentSettingsEnum;
...
class PlanfixAppartmensValuesCast implements Cast
{
    ...
    public function cast(DataProperty $property, mixed $value, array $context): mixed
    {
        try {
            ...
            foreach ($context as $name => $value) {
                if (!empty(ApartmentSettingsEnum::FIELDS()[$name]['name']))
    ...
    protected function getField($name, $value, $settings = 'FIELDS'): ExtractApartmentData
    {
        $const = ApartmentSettingsEnum::$settings();

        return ExtractApartmentData::from(
            [
    ...
    protected function getCustomField($value)
    {
        foreach ($value as $cName => $cValue) {
            ...
            if (!empty(ApartmentSettingsEnum::CUSTOM_FIELDS()[$fieldName])) {
                if ($fieldName != 'Объект (квартира)') {
                    ...
                    foreach (ApartmentSettingsEnum::FLAT_FIELDS() as $name => $flat) {
